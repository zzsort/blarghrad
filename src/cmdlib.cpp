...
void SetQdirFromPath_v2(char *path)
{
    ...
    const char *local_418[6] = { "baseq2\\","baseq2/","base\\","base/","main\\","main/" };

    if (((*path != '/') && (*path != '\\')) && (path[1] != ':')) {
        Q_getwd((char *)local_400);
        strcat(local_400, path);
    }
    ...
    if (pcVar7 != path) {
        ...
        while ((*pcVar7 != '/' && *pcVar7 != '\\')) {
            pcVar7--;
            _Count++;
            if (pcVar7 == path) goto LAB_00402654;
        }
        for (local_424 = 0; local_424 < 6; local_424++)
        {
            if ((game == 0) || (game == local_424 / 2 + 1)) {
                pcVar10 = local_418[local_424];
                if (!Q_strncasecmp(pcVar7 + 1, pcVar10, strlen(pcVar10))) {
                    strncpy((char *)local_400, path, (size_t)_Count);
...
LAB_00402654:
    ...
    Error("ERROR: could not locate game dirs\n");
    return;
}

char s_szQdirPath[1024];
char* SetQdirFromPath_v3(char *path)
{
    if (((*path == '/') || (*path == '\\')) || (path[1] == ':')) {
        strcpy(s_szQdirPath, path);
        return s_szQdirPath;
    }
    Q_getwd(s_szQdirPath);
    strcat(s_szQdirPath, path);
    return s_szQdirPath;
}
