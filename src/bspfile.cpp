...
// models
	for (i=0 ; i<nummodels ; i++)
	{
		...
		for (j=0 ; j<3 ; j++)
		{
			d->mins.data[j] = LittleFloat(d->mins.data[j]);
			d->maxs.data[j] = LittleFloat(d->maxs.data[j]);
			d->origin.data[j] = LittleFloat(d->origin.data[j]);
		}
	}
...
//
	...
	{
        dvertexes[i].point.x = LittleFloat(dvertexes[i].point.x);
        dvertexes[i].point.y = LittleFloat(dvertexes[i].point.y);
        dvertexes[i].point.z = LittleFloat(dvertexes[i].point.z);
	}
...
//
	for (i=0 ; i<numplanes ; i++)
	{
        ...
        dplanes[i].normal.y = LittleFloat(dplanes[i].normal.y);
        dplanes[i].normal.z = LittleFloat(dplanes[i].normal.z);
